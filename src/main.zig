...
pub fn main() !void {
    ...
    cube_mesh.transform.scale = storage.V3{ .x = 200, .y = 200, .z = 300 };
    ...
    text_mesh.transform.position = storage.V3{ .x = 50, .y = 900, .z = 0 };
    ...
    while (window.loop()) {
        ...
        meshbuf.buffer[3].transform.rotation.y += 0.001;
        // meshbuf.buffer[2].transform.rotation.z += 0.005;
        // meshbuf.buffer[3].transform.position.x = @floatFromInt(window.f.x);
        // meshbuf.buffer[3].transform.position.y = @floatFromInt(window.f.y);
